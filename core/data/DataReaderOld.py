...
class DataReader(DataController):
	...
	def GetImageDataForBaseAndExtension(self, fileName, extension):
		...
		if extension == DataReader.TypeMHA or extension == DataReader.TypeMHD:
			...
			b = a[:,:,:,0]
			VTK_data = numpy_to_vtk(num_array=b.ravel(), deep=True, array_type=VTK_FLOAT)
			...
			#imageData.SetScalarTypeAsDouble()
			...
			for z in range(int(dimensions[2])):
				for y in range(int(dimensions[1])):
					for x in range(int(dimensions[0])):
						magnitude = float(a[x,y,z,0]*a[x,y,z,0]+a[x,y,z,1]*a[x,y,z,1]+a[x,y,z,2]*a[x,y,z,2]);
						imageData.SetScalarComponentFromFloat(x, y, z, 0, math.sqrt(magnitude))
						dataIndex += 1
			########################
			return imageData
